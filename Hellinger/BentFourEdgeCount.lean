...
namespace Hellinger.BentFourEdgeCount
open Hellinger.Fourier Hellinger.BentFour

/-- Number of negative cross-term signs, each corresponding to one actual graph edge. -/
def negativeEdges (e : Fin 6 → ℝ) : ℕ := ∑ i, if e i = -1 then 1 else 0
...
theorem derivative_moment_edge_count (e : Fin 6 → ℝ)
    (he : ∀ i, e i ^ 2 = 1) (hnd : EdgeNondegenerate e) (ρ : ℝ) :
    2 ≤ negativeEdges e ∧ (∑ s : Cube 4, noiseDerivativeSum e ρ s ^ 2) =
      256 * BentEstimates.fourthMomentExpression (negativeEdges e) (ρ ^ 2) := by
  ...
  · exfalso
    ...
    norm_num at hz
  · constructor
    · simp only [negativeEdges, Fin.sum_univ_succ]
      dsimp
      norm_num
    simp only [negativeEdges, Fin.sum_univ_succ]
    dsimp
...
#print axioms derivative_moment_edge_count
end Hellinger.BentFourEdgeCount
